    ...
    try {
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            records.push({
                ...
                achievement: rec.getValue('achievement') || '',
                earned_on: rec.getValue('earned_on') || '',
        ...
        gs.error(LOG_PREFIX + 'resolveUserAchievements: ' + e.message)
        throw e
    }
}
...
export function resolveUserAchievement(env: any) {
    try {
        ...
        gs.error(LOG_PREFIX + 'resolveUserAchievement: ' + e.message)
        throw e
    }
}
...
export function resolveUserAchievementContent(env: any) {
    try {
        ...
        gs.error(LOG_PREFIX + 'resolveUserAchievementContent: ' + e.message)
        throw e
    }
}
    ...
    try {
        ...
        if (results && results.length > 0) {
            ...
            return {
                ...
                name: rec.getValue('name') || '',
                description: rec.getValue('description') || '',
                achievement_type: rec.getValue('achievement_type') || '',
                badge: rec.getValue('badge') || '',
                points: parseInt(rec.getValue('points') || '0', 10),
                active: rec.getValue('active') === 'true',
                sys_created_on: rec.getValue('sys_created_on') || '',
                sys_updated_on: rec.getValue('sys_updated_on') || '',
            }
        ...
        gs.error(LOG_PREFIX + 'resolveUserAchievementAchievement: ' + e.message)
        throw e
    }
}
